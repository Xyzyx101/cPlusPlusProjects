...
int Tree::DisplayQuestions() const
{
	DisplayQuestions( m_pRoot, 1 );
	return 0;
}

int Tree::DisplayQuestions(const Node * const pNode, int count) const
{
	if( pNode == nullptr )
	{
		return 0;
	...
	{
		count = DisplayQuestions( pNode->m_pYes , count);
	}
	if( pNode->m_pNo != nullptr )
	{
		count = DisplayQuestions( pNode->m_pNo , count);
	}
	return count;
}